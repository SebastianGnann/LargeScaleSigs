function [] = plotMapCL(lat,lon,z,varargin)
%plotMapCL Plots CL map with dots coloured according to an attribute.
%   Options:
...
%   figure_name: name for saving, e.g. 'CL_map'
...
addParameter(ip, 'figure_name', 'map_CL', @ischar)
...
ax = axesm('MapProjection','mercator','MapLatLimit',[-57 -16],'MapLonLimit',[-76 70]);
shapefile = shaperead('chile_50m.shp', 'UseGeoCoords', true);
...
c = colorbar;
c.Position = [0.65 0.3 0.028 0.15]; 
...
%% save fig
if save_figure
    saveFig(fig,strcat('CL_',figure_name),figure_path,figure_type)
end
