...
%     end
    violinplot(tmp,attributes.country)
    view(90,90)
    ylabel(signature_names_Groundwater{i})
    xticklabels({'US','GB','AUS','BR'})
    if mod(i+3,4)~=0
        set(gca,'XTickLabel',[]);
    end
%     xlim([quant_mat(i,1) quant_mat(i,5)])
	ylim([quant_mat(i,1) quant_mat(i,5)])
%     set(gca,'YTickLabel',[]);
end
% leg = legend('US','GB','AUS','BR','box','off');
% leg.Position = [.55 .14 .10 .11];
% yleg.Position = [0.89 0.15 0.10 0.11];
...
%     tmp(attributes.frac_snow>.2) = NaN;
    ...
    for j = 1%1:4
        ...
        rho_s(i) = corr(var(attributes.country==j),tmp(attributes.country==j),'type','Spearman','rows','complete');
    end
%     rho_s(i) = corr(var,tmp,'type','Spearman','rows','complete');
    title(strcat('\rho_s =','{ }',num2str(round(rho_s(i),2))))
