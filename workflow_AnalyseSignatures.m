...
tmp2 = CAMELS_signatures_Groundwater.RecessionParameters(:,1);
tmp2(tmp2>1) = NaN;
...
leg.Position = [0.760 0.123 0.101 0.113];
...
ranges = [-0.4 0.8; 0 1; 0 1; 0 1; ...
    ...
    0 150; 0 10];
colour_mat = [brewermap(4,'Spectral') .8*ones(4,1)];
fig = figure('pos',[100 100 700 300]);
...
fig = figure('pos',[100 100 700 600]);
...
leg = legend('US','GB','AUS','BR','box','off');
leg.Position = [0.778 0.114 0.101 0.11];
