...
%% plot maps
% make sure that TOSSH is not loaded so that TOSSH plotting code isn't used

plotMapUS(attributes.gauge_lat,attributes.gauge_lon,...
...
xlabel('Date')
xlim([datetime(1989,04,01) datetime(1999,04,01)])
ylabel('Q [mm/d]')
...
nr = 4;
scatter(attributes.aridity(attributes.country==nr),...
    CAMELS_signatures_OverlandFlow.SE_effect(attributes.country==nr))
scatter(attributes.aridity(attributes.country==nr),...
    CAMELS_signatures_OverlandFlow.IE_effect(attributes.country==nr))
